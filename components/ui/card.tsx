...
export function Card({ className = "", ...props }: CardProps) {
  return (
    <div
      className={`rounded-[1.75rem] border border-[color:var(--color-surface-overlay)] bg-[color:var(--color-surface-overlay-strong)] shadow-[0_20px_60px_rgba(var(--color-shadow-rgb),0.08)] backdrop-blur ${className}`.trim()}
      {...props}
    />
  );
}
